...
const show = (id) => {;
  Movement.findById(id)
    .then(movement => movement ? console.log(movement.toJSON()) : console.log('Not Found'))
    .catch(console.error)
    .then(done);
  };
...
const destroy = (id) => {
  Movement.findById(id)
   .then(movement => movement ? movement.remove() && console.log('removed') : console.log('Not Found'))
   .catch(console.error)
   .then(done);
};
