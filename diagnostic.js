...
// since the maxlength is defined in the schema, should that make sure that the length limited?
...
const show = (id) => {
  Movement.findById(id).then(function(movement) {
    // I think I should have an if statement here in to catch whether the movement already exists
    if (movement.!exists?) {
...
db.once('open', function() {
  ...
  switch (command) {
    case 'create':
      ...
      let ancestor = process.argv[7];
      if (true || name) {
        create(name, description, startYear, endYear, ancestor);
      } else {
        console.log('usage c <name> <description> <startYear>, <endYear>, <ancestor>');
        done();
      }
      break;
